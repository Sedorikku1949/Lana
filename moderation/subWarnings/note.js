...
module.exports = async(message, data) => {
    if (data.sanctions.notes <= 0) return message.channel.send({ embed: { color: '#FF6565', title: "Cette utilisateur n'a jamais recu de notes"}})
    let [x, embed] = [ 0 , data.sanctions.notes.map((e, index) => ({
        color: '#FF6565',
    ...
    if (data.sanctions.notes.length > 1) {
        await msg.react("◀️")
        await msg.react("▶️")
        await msg.react("🗑️")
    }
