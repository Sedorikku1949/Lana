...
module.exports = async(message, data) => {
    if (data.sanctions.ban <= 0) return message.channel.send({ embed: { color: '#FF6565', title: "Cette utilisateur n'a jamais été banni"}})
    let [x, embed] = [ 0 , data.sanctions.ban.map((e, index) => ({
        color: '#FF6565',
    ...
    if (data.sanctions.ban.length > 1) {
        await msg.react("◀️")
        await msg.react("▶️")
        await msg.react("🗑️")
    }
