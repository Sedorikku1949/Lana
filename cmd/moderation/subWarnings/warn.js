...
module.exports = async(message, data) => {
    if (data.sanctions.warn <= 0) return message.channel.send({ embed: { color: '#FF6565', title: "Cette utilisateur n'a jamais été averti"}})
    let [x, embed] = [ 0 , data.sanctions.warn.map((e, index) => ({
        color: '#FF6565',
    ...
    if (data.sanctions.warn.length > 1) {
        await msg.react("◀️")
        await msg.react("▶️")
        await msg.react("🗑️")
    }
