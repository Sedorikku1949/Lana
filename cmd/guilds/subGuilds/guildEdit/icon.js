...
module.exports = async(message, guild, guildList, question) => {
    await question.edit({ embed: { color: "#2C2F33", description: "**Quel seras la nouvelle pp de la guild ?**" } })
    ...
    c.on("collect", async(msg) => {
        ...
        guildList[index].icon = icon.attachment
        fs.writeFileSync("guild.json", JSON.stringify(guildList, null, 1))
        await deleteCache([`../../../../guild.json`])
        await msg.delete()
        await question.edit({ embed: { color: "#2C2F33", description: `**L'icon de la guild est désormais :**`, image: { url: icon.attachment } } })
        c.stop()
    })
