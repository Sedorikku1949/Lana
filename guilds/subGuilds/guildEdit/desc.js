...
module.exports = async(message, guild, guildList, question) => {
    await question.edit({ embed: { color: "#2C2F33", description: "**Quel seras la nouvelle description ?**" } })
    ...
    c.on("collect", async(msg) => {
        ...
        guildList[index].description = desc
        fs.writeFileSync("guild.json", JSON.stringify(guildList, null, 1))
        await deleteCache([`../../../../guild.json`])
        await message.channel.send({ embed: { color: "#2C2F33", description: `**La description de la guild ${guild.name} est désormais :** \`\`\`\n${desc}\`\`\`` } })
        c.stop()
    })
